# Continual ViT: Task-Routed LoRA Experts + a Fixed ETF Classifier
...
Fine-tuning a pretrained ViT on new classes overwrites the shared representation the old classes depended on — the textbook definition of catastrophic forgetting. The usual fixes (experience replay, distillation, regularizing the whole network) either need access to old data or trade a lot of plasticity for stability. This project explores a parameter-isolation approach instead: freeze everything that matters, give each task its own small, disposable set of weights, and only worry about *how those weights are combined* at inference.
...
An ImageNet-pretrained `vit_base_patch16_224` (via `timm`) is loaded once and every original parameter is frozen for the rest of training. It never sees a gradient again — it's pure shared feature extraction.
...
- **During training**, only the current task's adapter is active — a standard, cheap task-incremental LoRA fine-tune.
- **At inference**, every adapter that has ever been learned (including frozen ones from past tasks) is evaluated in parallel, and a softmax over **cosine similarity between each adapter's CLS-token delta and the frozen backbone's own CLS output** decides how much weight each expert gets. This is the "Mixture-of-Experts" part — but the router isn't a trained network, it's a zero-parameter similarity heuristic computed on the fly.
...
**Strengths**
- **Zero-parameter inference-time router.** Task discrimination at inference comes from cosine similarity on features that already exist — no gating network to train, no risk of gate collapse, fully interpretable (you can literally print the gate weights per image).
- **A classifier that can't drift.** Because the ETF head has no trainable parameters, there's structurally no way for it to develop the "new classes dominate the logits" bias that plagues linear-head continual learning — the separation between classes is decided once, geometrically, before any data is seen.
...
- **A genuinely controlled ablation.** Branching both Task-2 runs from the identical Stage-1 checkpoint isolates the orthogonality weight as the single variable under test, which is the right way to attribute any behavioral difference to the hyperparameter rather than to training noise.

**Current limitations, and what they point to next**
...
- **Only two tasks so far.** The 50/52 split is a clean, controlled setting to validate the mechanism before scaling to a longer task stream, where adapter count (and therefore inference-time compute) grows linearly — worth profiling before scaling out.
- **Both ablations were stopped early.** This was a deliberate choice to get fast comparative signal on the orthogonality weight rather than spending the full epoch budget on both variants before knowing which was worth continuing — the logical next step is running the more promising configuration to full convergence.

## Repository layout

```
├── ViT_Caltech101_EVT_V2_MOE_DoubleTask.ipynb   # full pipeline: data, model, training, both ablations
└── README.md
```

## Roadmap

- [ ] Run both Task-2 ablations to full convergence and report head-to-head final Task-1 retention vs. Task-2 accuracy
- [ ] Per-class / per-layer forgetting breakdown instead of aggregate accuracy only
- [ ] Learned gating network as a drop-in replacement for the cosine-similarity router
- [ ] Extend beyond two tasks to a longer class-incremental stream
- [ ] Report backward transfer (BWT) and forward transfer (FWT) as standard continual-learning metrics alongside raw accuracy